...
s1=MSUTorchDataset(90,10, 20, 30, 30, 40, 70, 9)
s1.clean_data()
...
class MSUTorchMetaDataViewer(MSUTorchMetaData): 

    def metadata(self):    
        print("Your metadata has been saved in the TorchSig Dataset MetaData Class")
        return Data_Options(
          fft_size=self.msu_metadata.fft_size,
           num_iq_samples_dataset=self.msu_metadata.num_iq_samples_dataset,
           num_signals_min=self.msu_metadata.num_signals_min, 
           num_signals_max=self.msu_metadata.num_signals_max, 
           snr_db_min=self.msu_metadata.snr_db_min, 
           snr_db_max=self.msu_metadata.snr_db_max, 
           class_list=self.msu_metadata.class_list)
    
    #def lookat(self):
     #   metadata=self.metadata()
      #  print(metadata)
        
MSU_metadata=MSUTorchMetaData(8192, 256, 1, 1, 30, 30, ["qpsk"])
MSU_viewer=MSUTorchMetaDataViewer(MSUTorchMetaData) #Giving the MSU_metadata to MorganTorchMetaDataViewer class
MSU_viewer.metadata()
#MSU_viewer.lookat()

#class MSUTorchSignalGenerator:
 # def __init__(self,MSUTorchMetaDataViewer): #Inherits from 
  #      self.msu_signalgenerator=MSUTorchMetaDataViewer
...
#def component_signals(self):
 ...
 #           return TorchSigIterableDataset(
  #               dataset_metadata=self.metadata(),
   ...
   # def signal(self):
     #   for i in range(5): 
    #        signal = next(self.dataset)

#signal=MSUTorchSignalGenerator()
#signal.component_signals()
